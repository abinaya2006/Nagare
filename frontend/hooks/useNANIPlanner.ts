...
export function useNANIPlanner() {
  ...
  const processNaturalLanguage = useCallback(async (text: string): Promise<NANIProcessResult> => {
    setProcessing(true);
    try {
      const { data } = await api.post("http://localhost:8000/orda/process", { message: text }, {
      ...
      return {
        ...
        segment: "afternoon",
        notes: data?.summary ?? "",
        insight: data?.summary ?? NANI_INSIGHTS[Math.floor(Math.random() * NANI_INSIGHTS.length)],
      };
    } catch {
  ...
  return { processNaturalLanguage, processInput: processNaturalLanguage, processing, isProcessing: processing, currentInsight, rotateInsight };
}

function localParse(text: string): NANIProcessResult {
  const lower = text.toLowerCase();
  let segment: NANIProcessResult["segment"] = "afternoon";
  if (lower.includes("tonight") || lower.includes("evening")) segment = "evening";
  else if (lower.includes("morning") || /\b[89](\s|am|:)/.test(lower)) segment = "morning";

  let priority: NANIProcessResult["priority"] = "medium";
  if (lower.includes("urgent") || lower.includes("important")) priority = "critical";
